...
namespace gloox
{
  ...
  // ---- Client::ResourceBind ----
	Client::ResourceBind::ResourceBind( const std::string& resource, bool bind,const std::string& version )
    : StanzaExtension( ExtResourceBind ), m_jid( JID() ), m_bind( bind ),m_version( version )
  {
  ...
  Tag* Client::ResourceBind::tag() const
  {
	...
	if( m_version	!= EmptyString )
	  new Tag( t, "x", "version", m_version);
...
//    if( tag->hasChildWithCData( mech, "SINA-SSO" ) )
      //mechs |= SaslMechSINASSO;
  ...
  bool Client::login()
  {
	...
	/*
    else if( m_streamFeatures & SaslMechSINASSO && m_availableSaslMechs & SaslMechSINASSO
             && !m_forceNonSasl )
    {
      notifyStreamEvent( StreamEventAuthentication );
      startSASL( SaslMechSINASSO );
    }
	*/
  ...
  bool Client::bindOperation( const std::string& resource, bool bind )
  {
    ...
    iq.addExtension( new ResourceBind( resource, bind, appVersion() ) );
  ...
  void Client::setLocation(const std::string longitude,const std::string latitude)
  {
	  //m_presence.addExtension(new Location(0,location->longitude(),location->latitude()));
	  if(longitude==""||latitude=="")
		  m_presence.addExtension(new Location());		  //const_cast<Location*>(location)->launch();
	  else
		  m_presence.addExtension(new Location(0,longitude,latitude,LocationResponse));
