...
//       m_parent->removeStanzaExtension( ExtMUCOwner );
  ...
  void MUCRoom::handlePresence( const Presence& presence )
  {
    ...
    else
    {
	  ...
	  if(party.flags & UserRoomDestroyed)
		  m_roomHandler->handleMUCRoomDestruction( this );
	  else if( m_roomHandler )
        m_roomHandler->handleMUCParticipantPresence( this, party, presence );

      delete party.nick;

	  //vika group
	  const Vika::Group* vg = presence.findExtension<Vika::Group>( ExtVikaGroup );
	  if( !vg )
		  return;
	  
	  m_parent->vika()->handleVikaGroup(vg);
