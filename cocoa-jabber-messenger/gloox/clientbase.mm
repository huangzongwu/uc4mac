...
namespace gloox
{
  ...
  ClientBase::ClientBase( const std::string& ns, const std::string& server, int port )
    : m_connection( 0 ), m_encryption( 0 ), m_compression( 0 ), m_disco( 0 ), m_vika(0), m_namespace( ns ),
      ...
      m_availableSaslMechs( SaslMechAll ), m_appversion( EmptyString ),
  ...
  ClientBase::ClientBase( const std::string& ns, const std::string& password,
    ...
    : m_connection( 0 ), m_encryption( 0 ), m_compression( 0 ), m_disco( 0 ), m_vika(0), m_namespace( ns ),
      m_password( password ),m_appversion( EmptyString ),
      m_xmllang( "en" ), m_server( server ), m_compressionActive( false ), m_encryptionActive( false ),
  ...
  void ClientBase::startSASL( SaslMechanism type )
  {
    ...
    switch( type )
    {
	  ...
	  /*
      case SaslMechSINASSO:
      {
        a->addAttribute( "mechanism", "SINA-SSO" );
		std::string tmp;
		tmp	= Base64::encode64( "" );
        a->setCData( tmp );
        break;
...
//         etc... see gssapi-sasl-draft.txt
  ...
  void ClientBase::send( Presence& pres , bool issafe )
  {
  ...
  bool ClientBase::keepAlive(std::string xmlTag)
  {
    if( m_connection && m_connection->state() == StateConnected )
    {
      if( m_compression && m_compressionActive )
        m_compression->compress( xmlTag );
      else if( m_encryption && m_encryptionActive )
        m_encryption->encrypt( xmlTag );
      else
        m_connection->send(xmlTag);

		logInstance().dbg( LogAreaXmlOutgoing, xmlTag );
		return true;
	}
	return false;
  ...
  std::list<std::string> ClientBase::getClientError()
  {
	  std::list<std::string> m_errorList;
	  std::string	m_errorinfo	= "";
	  switch(this->m_streamError)
	  {
  ...
  void ClientBase::registerVikaHandler( VikaHandler *vh )
  {
	  if( vh )
	  {
		  this->m_vika->registerVikaHandler(vh);
		  //m_vikaHandler	= vh;
	  }
  }

  void ClientBase::removeVikaHandler()
  {
	  m_vika->removeVikaHandler();
	  //m_vikaHandler	= 0;
  }
